...
export default function CertificateEdit() {
  ...
  const initialTags = [post?.tag1, post?.tag2].filter(Boolean) as string[];

  const [photoFile, setPhotoFile] = useState<File | null>(null);
  const [photoUrl, setPhotoUrl] = useState<string | null>(post?.imgUrl ?? null);
  ...
  const canSubmit = photoUrl !== null && tags.length >= 1;
  ...
  return (
    <div className={styles.page}>
      <div className={styles.uploadHeader}>
        ...
        <span className={styles.headerTitle}>인증샷 수정</span>
      ...
      <div className={styles.inner}>
          ...
          {photoUrl ? (
            <div className={styles.photoPreview}>
              ...
              <div className={styles.photoOverlay}>
                <button className={styles.trashBtn} onClick={() => { setPhotoUrl(null); setPhotoFile(null); }}>
                  <img src={TrashButton} alt="사진 삭제" />
                </button>
              </div>
            </div>
          ) : (
            <button className={styles.photoPlaceholder} onClick={() => fileInputRef.current?.click()}>
        ...
        <div className={styles.tagSection}>
          ...
          <div className={styles.tagHint}>
            <img src={tagError ? ErrorIcon : ErrorIconGray} alt="" className={styles.tagHintIcon} />
